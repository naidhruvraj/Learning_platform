...
//           setQuizStartedAt(Date.now());
//         } else {
//           console.error("API did not return a valid array of questions:", data);
//           setQuestions([]);
//         }
//       } catch (error) {
//         console.error("Error fetching questions:", error);
...
const MCQAssessment = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      setLoading(true);
      try {
        // Updated API call to your new FastAPI backend route
        const res = await fetch("http://localhost:8000/api/preassessment");
        const data = await res.json();
        
        // Accessing the questions array correctly from the FastAPI response
