...
const TeacherAssessment = () => {
  ...
  const handleExportExcel = () => {
    const data = [];
    students.forEach(s => {
      if (s.postAssessmentResults?.length > 0) {
  ...
  const getCardBackground = (index) => {
    const options = ['bg-gradient-to-tr from-white via-indigo-50 to-white', 'bg-gradient-to-tr from-white via-green-50 to-white', 'bg-gradient-to-tr from-white via-yellow-50 to-white'];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-200 to-blue-300 pb-10">
      ...
      <div className="mt-20 px-4 md:px-12 lg:px-24">
        <h1 className="text-4xl font-bold text-center mb-6 text-gray-900">📊 Students' Assessment Results</h1>
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <input
            type="text"
            placeholder="Search by student name..."
            className="p-3 rounded-xl bg-white border-2 border-indigo-400 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 w-full md:w-1/3"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
          />
          <select
          ...
          >
            <option value="">No Filter</option>
            {allModules.map((mod, idx) => <option key={idx} value={mod}>{mod}</option>)}
          </select>
          <button onClick={handleExportExcel} className="bg-green-500 hover:bg-green-600 text-white font-semibold px-4 py-2 rounded-lg shadow w-full md:w-auto">Export to Excel</button>
        </div>

        {/* No records found */}
        {filteredStudents.length === 0 ? (
          <p className="text-center text-red-600 font-semibold text-lg mt-10">No records found.</p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
            {filteredStudents.map((student, idx) => (
              <div
                ...
                className={`cursor-pointer transition-transform transform hover:scale-[1.01] p-6 rounded-xl shadow-xl border-4 ${expandedCard === idx ? 'border-indigo-400' : 'border-transparent hover:border-blue-300'} ${getCardBackground(idx)}`}
              >
